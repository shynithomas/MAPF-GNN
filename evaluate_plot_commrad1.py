...
MAX_STEPS = 70  # Timeout threshold to prevent infinite loops For 16x16 =30, 32x32 =50 48x48= 70
...
class MapfInferenceEnv:
    ...
    def step(self, actions, steps):
        ...
        for i in range(self.num_agents):
            ...
            head_i = self._get_occupied_nodes(i)[-1]
            for j in range(i + 1, self.num_agents):
                if self.finished[j]: continue
                
                # Edge Collision (Head-to-head swap)
                body_j = self._get_occupied_nodes(j)
        ...
        for i in range(self.num_agents):
            ...
            if self.timers[i] <= 0.001: 
                ...
                if self.current_nodes[i] == self.agents_objs[i].goal:
                    ...
                    #print(f" agent {i} finished time {self.global_time}")
        #print(f"global {self.global_time }")
...
def evaluate_scenario(model_path, map_file, scen_file, num_agents):
    ...
    #print(f"makespan {m_makespan}")
    #m_soc = sum([env.paths[i][-1][1] for i in range(env.num_agents)]) if done else float('inf')
    m_soc, individual_costs = compute_model_total_cost(env, done)
    ...
    #print("="*60)
    return m_success, time_marl, e_succ, time_expert, m_collisions, m_makespan, m_soc, e_makespan, e_soc

def compute_model_total_cost(env, done):
    """
    Computes the Sum of Costs (SOC) for all agents in the model-generated solution.
    
    Args:
        env: The MapfInferenceEnv instance after the simulation has run.
        done: Boolean indicating if all agents successfully reached their goals.
        
    Returns:
        total_cost (float): The Sum of Costs.
        agent_costs (dict): A dictionary mapping agent_id to its individual cost.
    """
    total_cost = 0.0
    agent_costs = {}

    # If the simulation timed out before finishing, standard practice is to 
    # either return infinity or heavily penalize the cost.
    if not done:
        return float('inf'), {}

    for i in range(env.num_agents):
        # env.paths[i] stores tuples of (node, t_in, t_out)
        # We get the very last tuple recorded for agent i
        last_step = env.paths[i][-1]
        #print(f" agent {i} path {env.paths[i]} \n last_step {env.paths[i][-1]}")
        # Index 1 is 't_in', the exact time the agent's head arrived at this node
        arrival_time = last_step[1] 
        
        # Verify the agent is actually at its goal
        current_node = last_step[0]
        goal_node = env.agents_objs[i].goal
        
        if current_node == goal_node:
            agent_costs[i] = arrival_time
            total_cost += arrival_time
            #print(f" agent{i} arrival time {arrival_time} total {total_cost}")
            
        else:
            # Failsafe: if 'done' was true but an agent isn't at its goal
            agent_costs[i] = float('inf')
            #print(f"Done true but agent {i} not at goal")
...
if __name__ == "__main__":
    ...
    MAP_FILE = '../maps/64room_006.map'
    SCEN_FILE = '../scenarios/64room_000.map.scen'
    NUM_AGENTS = 45
    result_text="Agents | ModelSuccessRate | ModelAvgTime | ModelAvgCollisions | ModelMakeSpan | ModelSOC \n"
    result_exp="Agents | ModelSuccessRate | ModelAvgTime | ModelAvgCollisions | ModelMakeSpan | ModelSOC \n"
    ...
    for no_agents in range(5,100,5):
        m_successes = 0
        e_successes = 0
        m_times = []
        e_times = []
        ...
        for scenario_idx in range(1,10, 20):
            ...
            print(f"===file {file}===agent{no_agents}====")
            m_succ, m_time, e_succ, e_time, m_collide, m_makespan, soc_m, e_makespan, soc_e = evaluate_scenario(MODEL_PATH, MAP_FILE, file, no_agents)
            ...
            m_ms.append(m_makespan)
            e_ms.append(e_makespan)
            m_soc.append(soc_m)
            e_soc.append(soc_e)
        ...
        m_avg_makespan = sum(m_ms)/len(m_ms)
        e_avg_makespan = sum(e_ms)/len(e_ms)
        m_avg_soc = sum(m_soc)/len(m_soc)
        e_avg_soc = sum(e_soc)/len(e_soc)
        ...
        summary_results['model_makespan'].append(m_avg_makespan)
        summary_results['model_soc'].append(m_avg_soc)
        summary_results['expert_makespan'].append(e_avg_makespan)
        summary_results['expert_soc'].append(e_avg_soc)
        ...
        print(f"{no_agents:<10} | {f'{m_successes}/{num_valid_scenarios} ({m_success_rate:.1f}%)':<15} | {m_avg_collisions:<15.4f} | "
              ...
              f"{m_avg_time:<15.4f} | {e_avg_time:<15.4f} | {m_avg_makespan} | {m_avg_soc}\n")
        result_text += str(no_agents)+ " | "+ str(m_success_rate)+" | "+str(m_avg_time)+" | "+str(m_avg_collisions)+" | "+ str(m_avg_makespan)+" | "+str(m_avg_soc)+"\n"
        result_exp += str(no_agents)+ " | "+ str(e_success_rate)+" | "+str(e_avg_time)+" | 0 | "+ str(e_avg_makespan)+" | "+str(e_avg_soc)+"\n"
             
    
    # Writing to the file.
   
    with open("Result_Config1_64room_000.txt", "w") as file:
        file.write(result_text)

    with open("Result_64room_000_XCBS.txt", "w") as file:
        file.write(result_exp)
    ...
    plt.tight_layout()
    plt.savefig('evaluation_results_1CommRadius_64room_000.png')
    print("\nEvaluation complete. Plots saved as 'evaluation_results_1CommRadius_Berlin_1_256.png'.")
    plt.show()    
