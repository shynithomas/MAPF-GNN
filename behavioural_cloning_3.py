...
class GeneralizedGraphActionGCN(nn.Module):
    ...
    def forward(self, v_seq, e_seq, s_feat, pos, edge_index, n_feats, mask):
        ...
        #print(f"goal emb{goal_emb}")
        # Fuse and process through GCN
        agent_state = F.relu(self.fusion(torch.cat([pos_emb, static_emb, goal_emb], dim=1)))
        #print(f"torch.cat {torch.cat([pos_emb, static_emb, goal_emb], dim=1)}")
        #print(f"self.fusion{self.fusion(torch.cat([pos_emb, static_emb, goal_emb], dim=1))}")
        #print(f"agent_state after relu{agent_state}")
        if edge_index.size(1) > 0:
            agent_state = F.relu(self.conv1(agent_state, edge_index))
            #print(f"after conv1 {self.conv1(agent_state, edge_index)}")
            #print(f"after conv1 relu {agent_state}")
            agent_state = F.relu(self.conv2(agent_state, edge_index))
            #print(f"after conv2 {self.conv2(agent_state, edge_index)}")
            #print(f"after conv2 relu {agent_state}")
          
        # Map agent state to available actions (neighbors)
        agent_state_expanded = agent_state.unsqueeze(1).expand(-1, n_feats.size(1), -1)
        #print(f"agent state expanded {agent_state_expanded}")
        combined = torch.cat([agent_state_expanded, n_feats], dim=2)
        #print(f"combined {combined}")
        logits = self.action_head(combined).squeeze(-1)
        #print(f"action head {self.action_head(combined)}")
        #print(f"logits {logits}")
        
        logits = logits.masked_fill(~mask, float('-inf'))
        #print(f"masked_fill logits {logits}")
...
if __name__ == "__main__":
    ...
    TRAINING_TASKS = [
        {"map": "../maps/empty-8-8.map", "scen": "../scenarios/empty-8-8.map-scen-even/empty-8-8-even-"},
        #{"map": "../maps/empty-16-16.map", "scen": "../scenarios/empty-16-16.map-scen-even/empty-16-16-even-"},
        #{"map": "../maps/empty-32-32.map", "scen": "../scenarios/empty-32-32.map-scen-even/empty-32-32-even-"},
        #{"map": "../maps/empty-48-48.map", "scen": "../scenarios/empty-48-48.map-scen-even/empty-48-48-even-"},
    ]

    EPOCHS_PER_TASK = 1
    env = RealTrafficEnvGeneral(body_len=1)
   ...
   #print(f"agent {agent.policy.parameters}")
    ...
    for scenario_idx in range(1,2,3):
        for agent_idx in range(2,3,2):
            if scenario_idx == 12:
                continue
            for task_idx, task in enumerate(TRAINING_TASKS):
                ...
                for start_agent_idx in range(2, 3):#10-agent_idx):
    ...
    # Save trained model
    '''
    MODEL_SAVE_PATH="BC_four_maps_20agents_20scen.pth"
    ...
    # Save the plot
    plt.savefig('LR_four_maps_20agents_20scen.png')
    print("Learning rate plot saved as 'LR_four_maps_20agents_20scen.png'")
    ...
    plt.savefig('TL_four_maps_20agents_20scen.png')
    print("Loss plot saved as 'TL_four_maps_20agents_20scen.png'")
